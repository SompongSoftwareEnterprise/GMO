    ...
    <div class="container">
        <div class="row section">
            ...
            <div class="col-xs-8 certificate text-center" style="margin-top: 55px;">
                <strong>CERTIFICATE FOR NON-GENETICALLY MODIFIED ORGANISM</strong>
        ...
        <div class="row section">
            <div class="col-xs-6 certificate">
                ...
                <p>{{ $certificateRequestForm['manufactory_name'] }}</p>
            </div>
            <div class="col-xs-6 certificate">
                <strong>ADDRESS</strong>
                <p>{{ $certificateRequestForm['manufactory_address1'] }}</p>
                <p>{{ $certificateRequestForm['manufactory_address2'] }}</p>
                <p>{{ $certificateRequestForm['manufactory_city'] }},
                {{ $certificateRequestForm['manufactory_province'] }},
                {{ $certificateRequestForm['manufactory_country'] }},
                {{ $certificateRequestForm['manufactory_zip'] }}</p>
            </div>
        </div>
        <div class="row section">
            <div class="col-xs-6 certificate">
                <strong>VENDOR OR CONSIGNEE</strong>
                <p>{{ $certificateRequestInfoForm['vendor_or_consignee'] }}</p>
            </div>
            <div class="col-xs-6 certificate">
                <strong>ADDRESS</strong>
                <p>{{ $certificateRequestInfoForm['address1'] }}</p>
                <p>{{ $certificateRequestInfoForm['address2'] }}</p>
                <p>{{ $certificateRequestInfoForm['city'] }},
                {{ $certificateRequestInfoForm['province'] }},
                {{ $certificateRequestInfoForm['country'] }},
                {{ $certificateRequestInfoForm['zip'] }}</p>
            </div>
        </div>
        <div class="row section">
            <div class="col-xs-12 certificate">
                <strong>DESCRIPTION OF PRODUCT</strong>
                <p>{{ $certificateRequestInfoForm['description_of_product'] }}</p>
                <!-- <p>THAILAND QUANTITY: 1,400 CARTONS</p> -->
                <!-- <strong>INVOICE NO AT. 11/81T</strong> -->
            </div>
        </div>
        <div class="row section">
            <div class="col-xs-4 certificate">
                <strong>COUNTRY OF ORIGIN</strong>
                <p>{{ $certificateRequestForm['origin_of_plant'] }}</p>
            </div>
            <div class="col-xs-4 certificate">
                <strong>FINAL DESTINATION</strong>
                <p>{{ $certificateRequestInfoForm['final_destination'] }}</p>
            </div>
            <div class="col-xs-4 certificate">
                <strong>PORT OF ENTRY OR EMBARKATION</strong>
                <p>{{ $certificateRequestInfoForm['port_of_entry'] }}</p>
            </div>
        </div>
